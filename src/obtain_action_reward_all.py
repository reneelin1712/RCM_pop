...
def evaluate_rewards(traj_data, policy_net, discrim_net, env, transit_dict, transit_data):
...
# Extract test and learner trajectories
test_traj = [row[0] for row in trajectory_data]
learner_traj = [row[1] for row in trajectory_data]
...
with open('./rewards/trajectories_with_actions_rewards_returns.csv', 'w', newline='') as csvfile:
...
with open('./rewards/trajectories_all_actions_rewards.csv', 'w', newline='') as csvfile:
    csv_writer = csv.writer(csvfile)
