...
def evaluate_model(target_od, target_traj, model, env, n_link=437):
    ...
    for ori, des in zip(target_o, target_d):
        # Reset environment for the current origin-destination pair
        curr_ori, curr_des = env.reset(st=ori, des=des)
        
        # Compute action probabilities from the model for the current destination
        des_ts = (torch.ones_like(state_ts) * curr_des).to(device)
        action_prob = model.get_action_prob(state_ts, des_ts).detach().cpu().numpy()  # n_link, n_actions
        state_action = env.state_action[:-1]
        action_prob[state_action == env.pad_idx] = 0.0  # Mask out invalid actions
        
        # Initialize the path for the current OD pair
        sample_path = [str(curr_ori)]
        curr_state = curr_ori
        
        # Generate the sample trajectory for the current OD pair
        for _ in range(50):  # Limit the trajectory to 50 steps
            # Take a step in the environment using the model's action probabilities
            action = np.argmax(action_prob[curr_state])  # Choose the most likely action based on the model
            ...
            if done:  # If the environment signals done, end trajectory generation
                break
    ...
    trajectory_data = []
    for test_path, learner_path in zip(target_traj, learner_traj):
